...
int float_f2i(unsigned uf) {
  ...
  int x = (uf&0x007fffff)^0x800000;           //尾数
  int order = (uf&0x7f800000)>>23;            //阶码
  if(order>158)
    return n;                               //溢出时返回溢出标志
  if(order<127)
    return 0;                               //绝对值小于1时，转换成整数为0
  else if((uf>>31&1)==1){                  //负
    if(order>150) 
      return ~(x<<(order-150))+1;             //尾数化整数
    else
      return ~(x>>(150-order))+1;             //负时转化成补码
  }
  else{
    if(order>150)
      return x<<(order-150);
